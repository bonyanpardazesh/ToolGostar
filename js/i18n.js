...
class I18n {
    ...
    translateGalleryPage(language) {
        ...
        // Update page title
        const currentTitle = document.title;
        if (currentTitle.includes('Gallery - ToolGostar') || currentTitle.includes('گالری - گروه صنعتی طول گستر')) {
            document.title = language === 'fa' ? 
                'گالری - گروه صنعتی طول گستر' : 
                'Gallery - ToolGostar Project Showcase';
            console.log(`📄 Page title updated to: ${document.title}`);
        }
        ...
        if (galleryIntroTitle) {
            galleryIntroTitle.innerHTML = language === 'fa' ? 'گالری پروژه‌ها' : 'Project Gallery';
